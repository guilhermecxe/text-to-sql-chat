...
class AgentExecutorService:
    def __init__(self, redis:Redis, agents: dict, langfuse_client: Langfuse, langfuse_handler: CallbackHandler):
        self._redis = redis
        self._agents = agents
        self._steps = 0
        self._calls = 0
        self._limit_of_calls_per_day = int(os.environ.get("DAILY_USAGE_LIMIT", 30))
    ...
    def _get_usage(self):
        ...
        with open("data/usage.json", "r") as f:
            ...
            # Mantendo apenas os últimos n registros
            n = 3
            sorted_usage = dict(sorted(
                usage.items(), 
    ...
    async def execute(self, job_id: str, agent_name: str, thread_id: str, user_prompt: str, debug: bool = False):
        ...
        steps = 0
        estimated_steps = ((self._steps / self._calls) + 1) if self._calls > 5 else 10

        try:
            with self._langfuse_client.start_as_current_observation(**observation_kwargs) as obs:
